...
const BOOKINGS_PAGE_PATH = "/admin/bookings/list.php";
...
try {
    ...
    if ($booking) {
        ...
        if ($status === "cancelled" && in_array($booking["status"], ["pending", "confirmed"], true)) {
            ...
            if ($booking["payment_status"] === "pending") {
                ...
                $paymentStmt->execute([
                    "transaction_ref" => "ADMIN-CANCEL-" . bin2hex(random_bytes(4)),
    ...
    $pdo->commit();
    setFlash("info", "Booking status updated.");
} catch (Throwable $e) {
